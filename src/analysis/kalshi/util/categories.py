...
# SQL expression to extract category from event_ticker (bare column, no alias)
CATEGORY_SQL = """
CASE
...
def category_sql(alias: str = "") -> str:
    """Return the CATEGORY_SQL expression with the given table alias.

    Use when event_ticker is referenced through a JOIN alias, e.g. category_sql("m").
    Pass alias="" (default) for bare column references, equivalent to CATEGORY_SQL.
    """
    col = f"{alias}.event_ticker" if alias else "event_ticker"
    return (
        f"CASE\n"
        f"    WHEN {col} IS NULL OR {col} = '' THEN 'independent'\n"
        f"    WHEN regexp_extract({col}, '^([A-Z0-9]+)', 1) = '' THEN 'independent'\n"
        f"    ELSE regexp_extract({col}, '^([A-Z0-9]+)', 1)\n"
        f"END"
    )
